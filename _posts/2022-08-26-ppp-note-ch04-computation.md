...
categories: [C/C++, PPP]
tags: [cpp, expression, symbolic constant, operator, type conversion, if statement, switch statement, while statement, block, for statement, break statement, continue statement, function, vector]
---
本章将介绍一些与计算相关的基本概念。
...
* C++11之后才有`{}`初始化语法
* `{}`初始化优先于`()`初始化，详见9.4.2节“C++初始化语法”。例如：
