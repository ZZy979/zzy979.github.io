...
Flink提供了两个**关系型** API：Table API和SQL，用于统一流式和批处理。Table API是一个语言集成查询API，支持Java、Scala和Python，能够以非常直观的方式使用选择、过滤和连接等关系运算符来组合查询。Flink SQL基于[Apache Calcite](https://calcite.apache.org/)，它实现了SQL标准。无论输入是连续的（流式）还是有界的（批处理），用两种接口编写的查询都具有相同的语义，输出相同的结果。

Table API和SQL以及DataStream API可以互相无缝集成。

Table API & SQL需要的依赖见2.2.2节。为了在IDE中运行Table API程序，还需要添加依赖`flink-table-planner_${scala.version}`。

### 4.1 基本概念和常用API
<https://nightlies.apache.org/flink/flink-docs-release-2.2/docs/dev/table/common/>
...
`TableEnvironment`是Table API和SQL程序的入口点，负责注册表、执行查询、管理UDF等。每个`Table`都绑定到一个特定的`TableEnvironment`。
...
* [Table API Connectors](https://nightlies.apache.org/flink/flink-docs-release-2.2/docs/connectors/table/overview/)

### 4.2 DataStream API集成
<https://nightlies.apache.org/flink/flink-docs-release-2.2/docs/dev/table/data_stream_api/>

动态表
<https://nightlies.apache.org/flink/flink-docs-release-2.2/docs/dev/table/concepts/dynamic_tables/>

Dynamic Table：流式数据在 Table API 中的抽象表示，随着新数据的到达而动态变化
