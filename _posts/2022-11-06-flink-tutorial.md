...
##### 3.3.1.1 map()
`map()`算子接收一个[MapFunction](https://nightlies.apache.org/flink/flink-docs-stable/api/java/org/apache/flink/api/common/functions/MapFunction.html)接口参数，对元素进行一对一转换，即每个元素对应恰好一个结果。由于`MapFunction`是函数式接口，因此可以使用Lambda表达式。
...
##### 3.3.1.2 flatMap()
`flatMap()`算子接收一个[FlatMapFunction](https://nightlies.apache.org/flink/flink-docs-stable/api/java/org/apache/flink/api/common/functions/FlatMapFunction.html)接口参数，对元素进行一对n转换，即每个元素映射到零个或多个结果。
...
`keyBy()`算子接收一个[KeySelector](https://nightlies.apache.org/flink/flink-docs-stable/api/java/org/apache/flink/api/java/functions/KeySelector.html)接口参数（元组也可以使用字段索引，POJO也可以使用字段名），返回`KeyedStream`。接口`KeySelector`的抽象方法从一个元素提取出用于分组的key。
...
`KeyedStreams`提供了更通用的聚合算子`reduce()`，以及求和、最大值和最小值的三个特例：
* `reduce()`：接收一个[ReduceFunction](https://nightlies.apache.org/flink/flink-docs-stable/api/java/org/apache/flink/api/common/functions/ReduceFunction.html)接口参数，对每个相同key的分组内的元素进行“滚动”聚合操作，即`a[0], a[1], a[2], ... -> a[0], f(a[0], a[1]), f(f(a[0], a[1]), a[2]), ...`
* `sum()`：计算前缀和
* `max()`：计算当前遇到的最大值
* `min()`：计算当前遇到的最小值
...
```java
class Deduplicator extends RichFlatMapFunction<Event, Event> {
...
public static void main(String[] args) throws Exception {
    ...
    DataStream<String> control = env.fromElements("DROP", "IGNORE").keyBy(w -> w);
    DataStream<String> streamOfWords = env.fromElements("Apache", "DROP", "Flink", "IGNORE").keyBy(w -> w);
...
假设要对事件流排序：在没个事件到达时就处理事件，输出按时间戳排序的流。但是发现存在一些困难：
...
##### 3.4.2.3 窗口函数
有三种方式处理窗口内的数据：
* 批处理，使用[ProcessWindowFunction](https://nightlies.apache.org/flink/flink-docs-stable/api/java/org/apache/flink/streaming/api/functions/windowing/ProcessWindowFunction.html)，接收一个包含窗口中所有元素的`Iterable`，可以全量计算；
...
public static void main(String[] args) throws Exception {
    ...
    DataStream<SensorReading> input = env.fromElements(
            ...
            new SensorReading(0, "b", 2),
            new SensorReading(30000, "b", 5),
            new SensorReading(120000, "a", 15),
...
```java
class MyWastefulMax extends ProcessWindowFunction<SensorReading, Tuple3<String, Long, Integer>, String, TimeWindow> {
    @Override
    public void process(
            String key,
            Context context,
...
```java
class MyReducingMax implements ReduceFunction<SensorReading> {
    @Override
...
```java
class MyWindowFunction extends ProcessWindowFunction<SensorReading, Tuple3<String, Long, Integer>, String, TimeWindow> {
    @Override
    public void process(
            String key,
            Context context,
...
注意`Iterable<SensorReading>`只包含一个元素，即`MyReducingMax`预先计算的最大值。
