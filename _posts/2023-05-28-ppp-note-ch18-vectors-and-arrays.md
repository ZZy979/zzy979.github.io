...
注：引用无法像空指针一样表达“不存在”的含义（相当于将检查空指针的工作留给了调用者）。C++17引入了`std::optional`，表示一个“可能存在的值”，同时有助于避免空指针问题。详见头文件[\<optional\>](https://en.cppreference.com/w/cpp/header/optional)。
