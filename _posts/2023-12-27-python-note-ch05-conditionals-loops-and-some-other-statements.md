...
注：
* 表达式`x + y`会调用`x.__add__(y)`，如果不存在或返回`NotImplemented`则尝试`y.__radd__(x)`。
* 表达式`x += y`等价于`x = x.__iadd__(y)`（该方法通常原地修改并返回`self`），如果不存在或返回`NotImplemented`则按上述方式计算`x + y`再赋值给`x`。
* 对于不可变类型（如`int`和`str`），`x += y`等价于`x = x + y`，结果是`x`会引用一个新对象。例如：
...
对于可变类型（如`list`），前者会原地修改`x`，而后者会让`x`引用一个新对象。例如：
