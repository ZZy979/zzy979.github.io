...
注意，使用SFINAE的意义在于避免不必要的模板实例化。如果不使用SFINAE（即直接使用第1节中的定义），假设传入两个`vector`参数，则编译器在重载解析阶段会选择该模板，而在模板实例化阶段才报错。相反，如果使用SFINAE，则编译器在重载解析阶段就会排除这个模板，并报错“没有匹配的重载”，节省了模板实例化的开销，错误消息更加明确。
