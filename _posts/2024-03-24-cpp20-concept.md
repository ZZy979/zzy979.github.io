...
#### 2.2.1 简单要求
简单要求(simple requirement)是任意表达式语句，断言该表达式是合法的（能够编译通过）。表达式并不会被求值，只检查语言正确性。例如：
...
#### 2.2.2 类型要求
类型要求(type requirement)是`typename`后面跟着一个类型名称，验证指定的类型存在。例如：
...
使用SFINAE可以避免不必要的模板实例化。如果不使用SFINAE（即没有任何约束），当实参不满足模板的要求时，编译器在重载解析阶段会选择该模板，而在模板实例化阶段才报错。相反，如果使用SFINAE，则编译器在重载解析阶段就会排除这个模板，并报错“没有匹配的重载”，节省了模板实例化的开销，错误消息也更加明确。

标准库提供了一些能够更方便地利用SFINAE的特性。

### 3.1 std::enable_if
C++11引入了类模板`std::enable_if`，定义如下：
...
也可以通过在后置返回类型中利用SFINAE实现对模板参数的约束：
...
例如，可以这样定义模板`is_iterable`来检查一个类型是否是“可迭代的”（即有成员函数`begin()`和`end()`）：
