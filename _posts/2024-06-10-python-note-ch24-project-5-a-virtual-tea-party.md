...
值得注意的是，本章使用的`asyncore`和`asynchat`模块已在Python 3.6中弃用、在3.12中移除。如果愿意，可以尝试使用第14章讨论的其他方法（例如分叉或线程），甚至使用新的`asyncio`模块重写这个项目（详见[《基于协程和asyncio的并发编程》]({% post_url 2025-11-30-python-coroutine-and-asyncio %})）。
