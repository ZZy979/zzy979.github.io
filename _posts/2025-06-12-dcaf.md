...
论文提出了一个动态算力分配框架(Dynamic Computation Allocation Framework, DCAF)，目标是在**算力预算约束下，对不同价值的流量进行差异化算力分配，从而最大化总收益**，同时考虑在线服务系统的稳定性。
...
* λ的含义：算力边际收益的约束（即“性价比”的临界值）。对于请求i，给定λ，若动作j<sub>1</sub>优于动作j<sub>2</sub>，则 $Q_{ij_1} - \lambda q_{j_1} > Q_{ij_2} - \lambda q_{j_2}$ ，即 $\frac{\Delta Q}{\Delta q} > \lambda$ ，如下图所示。
