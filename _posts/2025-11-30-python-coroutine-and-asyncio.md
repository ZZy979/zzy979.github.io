...
### 1.2 事件循环
**事件循环**(event loop)是`asyncio`的核心组件，负责调度和执行协程。事件循环包含一个待运行作业的“队列”，每次从队列中取出一个作业并给予它控制权，然后该作业就会运行。一旦该作业暂停或完成，就把控制权返回给事件循环，然后事件循环会选择另一个作业。这个过程将无限重复。（可以把事件循环大致类比为线程池的调度线程。）
...
任务还维护了一个回调函数列表，其重要性将在下一节中讨论。需要注意的是，任务本身不会被添加到事件循环中，只有任务的回调函数才会。
...
与任务不同，**等待协程并不会将控制权交还给事件循环**。`await coroutine`的行为实际上与调用普通函数相同。不过，如果一个协程等待另一个协程，该协程又等待一个任务或future，那么这两个协程都会暂停，控制权仍然会交还给事件循环。
...
[异步迭代器](https://docs.python.org/3/reference/datamodel.html#asynchronous-iterators)(asynchronous iterator)是提供了`__anext__()`方法的对象。该方法可以调用异步代码，必须返回一个可等待对象（例如返回一个future，或者直接使用`async def`定义以返回协程对象），其结果是迭代器的下一个值，当迭代结束时应该引发`StopAsyncIteration`。内置函数`anext(ait)`等价于`ait.__anext__()`。
...
class DelayedCounter:
    ...
    async def __anext__(self):
        ...
        return await asyncio.sleep(self.delay, result=self.value)
```

这个迭代器会生成整数序列0~end-1，两个值之间间隔指定的秒数。也可以将`__anext__`定义为普通方法，直接返回协程对象`asyncio.sleep()`：
...
def __anext__(self):
    ...
    if self.value >= self.end:
        raise StopAsyncIteration
    return asyncio.sleep(self.delay, result=self.value)
```

[异步生成器](https://docs.python.org/3/reference/datamodel.html#asynchronous-generator-functions)是使用`async def`定义且包含`yield`语句的函数或方法。调用它会返回一个异步迭代器对象。例如，上面的`DelayedCounter`可以改写成等价的异步生成器：

```python
async def delayed_counter(end, delay=1):
    for value in range(end):
        yield await asyncio.sleep(delay, result=value)
```

`async for`语句可以方便地对异步可迭代对象进行迭代，只能在协程函数内使用。语法如下：
...
```python
async def main():
    counter = DelayedCounter(10, delay=1)
    # print 0~9, one value per second
    async for value in counter:
        print(value)
```

### 1.6 异步上下文管理器和async with语句
[异步上下文管理器](https://docs.python.org/3/reference/datamodel.html#asynchronous-context-managers)(asynchronous context manager)是提供了`__aenter__()`和`__aexit__()`方法的对象，这两个方法必须返回一个可等待对象。异步上下文管理器可以用于`async with`语句。
...
## 2.工作原理
本节将介绍协程的控制权转移机制的工作原理，以及如何自定义异步运算符。
...
`__step()`方法的核心逻辑如下：
* 对包装的协程`self._coro`调用`send(None)`，使其开始或恢复执行。
...
Python的`await`关键字针对协程做了特殊优化，对应的C函数如下（源代码见[Python/ceval.c#L3944](https://github.com/python/cpython/blob/main/Python/ceval.c#L3944)）：
...
其中调用的`_PyCoro_GetAwaitableIter()`函数返回给定对象的“可等待迭代器”（源代码见[Objects/genobject.c#L1067](https://github.com/python/cpython/blob/3.14/Objects/genobject.c#L1067)）：
...
另外，PEP 492要求可等待对象的`__await__()`方法返回一个迭代器。为了保持语义兼容，协程对象也有`__await__()`方法（源代码见[Objects/genobject.c#L1116](https://github.com/python/cpython/blob/3.14/Objects/genobject.c#L1116)）：
...
该方法创建并返回了一个协程包装器类型(`PyCoroWrapper`)的对象。包装器对象也有`send()`、`throw()`和`close()`方法，分别直接调用底层协程的对应方法。另外，包装器对象实现了迭代器协议，其`__next__()`方法等价于`send(None)`（源代码见[Objects/genobject.c#L1282](https://github.com/python/cpython/blob/3.14/Objects/genobject.c#L1282)）：
...
其中，`gen_send()`对应生成器/协程对象的`send()`方法，`gen_iternext()`对应生成器的`__next__()`方法（等价于`gen_send(obj, NULL)`）。因此，显式调用协程对象的`__await__()`并对返回的对象调用`send()`方法也等价于调用协程对象本身的`send()`方法。由于协程的`__await__()`方法不包含`yield`，同样不会使当前协程暂停。
...
## 3.asyncio模块
Python标准库模块`asyncio`是用于编写异步代码的库，特别适合于I/O密集型操作以及构建高性能异步框架，例如网络请求、Web服务器、数据库连接、实时消息队列处理等。
...
| `sleep(delay, result=None)` | 异步休眠 | `await asyncio.sleep(1)` |
...
| `start_server(cb, host, port)` | 启动TCP服务器 | `server = await asyncio.start_server(handle_echo, '0.0.0.0', 8888)` |
