...
## 4.6 复合类型
有时，需要表达一个对象的类型同时是其他几种类型的子类型。在Scala中，这可以借助**复合类型**(compound type)（也称为**交集类型**(intersection type)）来表达。

例如，假设有两个特质`Cloneable`和`Resetable`：
...
下面的方法接受一个同时实现了这两个接口的参数，其类型声明为复合类型`Cloneable with Resetable`：
...
复合类型也可以包含两种以上类型：`A with B with C with ...`。
