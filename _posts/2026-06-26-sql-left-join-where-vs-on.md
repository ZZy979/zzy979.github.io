...
优化器实际执行时可能通过谓词下推等方式调整执行顺序，但最终结果必须与逻辑语义一致。
...
| Bob | 35 | NULL | NULL | ❌订单创建时间不是2026-06-01 |
...
虽然条件`Users.age >= 30`的本意是对用户进行前置过滤，但`WHERE`子句是在`LEFT JOIN`完成后统一执行的，不会在`JOIN`前后分别执行一部分。不过，现代SQL优化器可能会做“谓词下推”优化，即在执行计划中将部分过滤条件提前执行。例如，`Users.age >= 30`可能会被下推到扫描`Users`表时执行，但这不会改变最终的语义结果。
...
结果是 "Cindy" 也被返回了。根本原因是`ON`子句仅决定如何匹配右表，无法起到过滤左表的作用。
